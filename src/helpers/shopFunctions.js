...
export const sumCart = (price) => {
  allValue += price;
  const spanAll = document.querySelector('.total-price');
  spanAll.innerText = `${allValue.toFixed(2)}`;
};
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async () => {
    const adicionaAoCarrinho = document.querySelector('.cart__products');
    saveCartID(id);
    const dados = await fetchProduct(id);
    const { pictures } = dados;
    const itemCard = {
      id,
      price,
      title,
    ...
    const produto = createCartProductElement(itemCard);
    adicionaAoCarrinho.appendChild(produto);
